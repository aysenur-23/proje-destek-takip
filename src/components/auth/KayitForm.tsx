...
export function KayitForm() {
  ...
  function sifreGuc(s: string): "zayif" | "orta" | "guclu" | null {
  ...
  const gucMetin: Record<string, string> = { zayif: "Zayıf", orta: "Orta", guclu: "Güçlü" };
  const gucRenk: Record<string, string> = {
    zayif: "text-red-500",
    orta: "text-amber-500",
    guclu: "text-emerald-600",
  };
  ...
  return (
    <div className="w-full max-w-sm">
      ...
      <div className="mb-7 text-center">
        ...
        <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Ücretsiz Başlayın</h1>
        <p className="mt-1.5 text-sm text-slate-500">Hesap oluşturun, filtrelemeye başlayın</p>
      </div>

      {/* Avantajlar */}
      <div className="mb-5 space-y-1.5 rounded-xl bg-emerald-50 border border-emerald-100 px-3.5 py-3">
        {[
          "40+ destek programı anlık filtreleme",
          "Kural tabanlı uygunluk analizi",
          "Premium ile AI analizine erişin",
        ].map((f) => (
      ...
      <form onSubmit={handleSubmit} className="card p-6 space-y-4">
          ...
          {guc && (
            <div className="mt-2 flex items-center gap-2.5">
              <div className="flex gap-1">
                {(["zayif", "orta", "guclu"] as const).map((s, i) => (
                  <div
                    key={s}
                    className={cn(
              ...
              <span className={cn("text-xs font-medium", gucRenk[guc])}>
                {gucMetin[guc]}
              </span>
            </div>
          )}
        ...
        <button
          type="submit"
          disabled={yukleniyor || !ad || !email || !sifre}
        ...
        <p className="text-center text-[11px] text-slate-400">
          Kayıt olarak{" "}
          <Link href="/gizlilik" className="underline hover:text-slate-600 transition-colors">
            Gizlilik Politikası
          </Link>
      ...
      <p className="mt-4 text-center text-sm text-slate-500">
        ...
        <Link href="/giris" className="font-semibold text-blue-600 hover:text-blue-700 transition-colors">
          Giriş yapın
        </Link>
      </p>
    </div>
